...
| 错误码 | 含义 | 触发场景 |
|--------|------|----------|
| 4001 | 图片数据无效 | Base64 解码后无法识别为图像，或 mask 与 CT 尺寸不一致 |
| 4002 | 掩膜为空 | mask 不包含任何前景像素（全为 0） |
| 4003 | 膨胀超出边界 | 膨胀后 ROI 覆盖了全部图像像素 |
| 4004 | Base64 解码失败 | 传入的字符串不是合法的 Base64 编码 |
